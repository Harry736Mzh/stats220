...
# Visualize walking duration distribution
ggplot(filter(latest_data, walking_status == "Yes"), 
       aes(x = factor(walk_duration))) +
  geom_bar(fill = "#3498db", alpha = 0.8, width = 0.7) +
  labs(x = "Walking Duration (hours)", y = "Number of People") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14),
        axis.text = element_text(size = 11),
        panel.grid.minor = element_blank())

# Visualize exercise preferences
exercise_data %>%
  count(exercise_type) %>%
  ggplot(aes(x = reorder(exercise_type, n), y = n)) +
  geom_col(fill = "#e74c3c", alpha = 0.8, width = 0.7) +
  coord_flip() +
  labs(x = "Exercise Type", y = "Number of People") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14),
        axis.text = element_text(size = 11))
